...
export const CreateTask = ({ todos, setTodos, setCurrentContainer }: { todos: any, setTodos: any, setCurrentContainer: any }) => {
    ...
    const defaultStartTime = moment().format('h:mm A');
    const defaultEndTime = moment().add(30, 'minutes').format('h:mm A');

    useEffect(() => {
        date ? setDate(date) : setDate(todayDate);
        startTime ? setStartTime(startTime) : setStartTime(defaultStartTime);
        endTime ? setEndTime(endTime) : setEndTime(defaultEndTime);
    }, [date, startTime, endTime, defaultStartTime, defaultEndTime, todayDate]);
    ...
    const handleAddTask = () => {
        ...
        const newTask = {
            ...
            title,
            date,
            duration: `${moment(startTime, 'h:mm A').format('h:mm A')} - ${moment(endTime, 'h:mm A').format('h:mm A')}`,
            completed: false,
        };
        ...
        <div className="w-full p-6 flex flex-col gap-8 border border-gray-100 rounded-lg shadow-xl">
            <div className='flex flex-col gap-4'>
                <div className='flex justify-between items-center'>
                    ...
                    <button
                        className="w-6 h-6"
                        onClick={() => setCurrentContainer('calendar')}
                    >
